---
description: Run a knowledge compilation pass (reinforce/decay/archive/graduate)
---

Run a knowledge compilation pass. Reinforcement is explicit and must be justified —
nothing is inferred from mere mentions.
...
2. Decide which `knowledge/**` articles were genuinely re-confirmed by that material
   ...
   and where.
3. Show a dry run and your justification for each reinforcement/graduation:
   ```bash
   tools/silo-kb/silo-kb compile --dry-run --reinforce <id-or-path>,... [--graduate <id>:projects/<proj>/<name>.md]
   ```
4. After the user confirms, run it for real (drop `--dry-run`), then:
   ...
   ```
5. Report what changed: reinforced, decayed, archived, graduated — with the log.md entry.
