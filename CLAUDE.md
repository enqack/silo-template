...
- **Every other `.md` file** needs YAML frontmatter with a required `type` field. OKF doesn't mandate a
  ...
  `title`, `description`, `resource` (a repo-relative path), `tags`, `timestamp`.
...
- `knowledge/**` is **working theory** — mutable, confidence-decaying. Required frontmatter beyond
  ...
  `reinforce_count`, `sources` (non-empty).
...
(`nomic-embed-text`). Manual build: `cd tools/silo-kb && go build -o silo-kb .`
...
| `silo-kb compile [--dry-run] [--reinforce …] [--graduate …]` | knowledge lifecycle run |
